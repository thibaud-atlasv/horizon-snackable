...
import { FISH_LEFT, FISH_RIGHT, WATER_SURFACE_Y, WATER_BOTTOM_Y, ZONE_FLOOR_Y } from '../Constants';
...
export class BubblePool extends Service {
  ...
  async onReady(): Promise<void> {
    for (let i = 0; i < POOL_SIZE; i++) {
      ...
      // Scatter at a random position in the play area to populate the background
  ...
  release(entity: Entity): void {
    ...
    if (tc)
      tc.worldPosition = this._poolPosition;
