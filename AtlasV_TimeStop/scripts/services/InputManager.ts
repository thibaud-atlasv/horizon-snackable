...
import { FallingObjService } from './FallingObjService';
import { Events, GamePhase, HUDEvents, ScoreGrade } from '../Types';
import { getPrecision } from '../Utils';

@service()
export class InputManager extends Service {
  ...
  private _handleTap(): void {
    ...
    const target = FallingObjService.get().getLowestFallingObj();
    if (!target) return;
    ...
    EventService.sendLocally(Events.BambooSlice, { worldX: target.getWorldX(), worldY: lowestY });
  ...
  private _afterFreeze(): void {
    const allFrozen = (
      ...
      this._spawnedCount > 0 &&
      FallingObjService.get().getActiveCount() === 0
    );
