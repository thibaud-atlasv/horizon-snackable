 ...
 * New zones are immediately filled when unlocked.
 */
@service()
export class FishSpawnService extends Service {

  // 0-indexed per zone: fish alive count and seconds until next spawn attempt
  ...
  private async _spawnInZone(zoneIndex: number): Promise<void> {
    ...
    EventService.sendLocally(
      Events.InitFish,
      { defId: def.id, speedMultiplier: 1.0, spawnX, baseY: spawnY, speedMin: def.speedMin, speedMax: def.speedMax },
      { eventTarget: entity },
    );
