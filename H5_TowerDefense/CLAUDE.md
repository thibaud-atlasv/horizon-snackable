# H5_TowerDefense — Claude Code Context
...
Mobile **portrait** single-player **tower defense** game built with **Meta Horizon Worlds SDK** (TypeScript).
...
Use `Service.inject()` to declare dependencies; access services via `MyService.get()`.
...
export class MyService extends Service {
  ...
  onRestart(): void {
    this._data.clear(); // reset state — never dispose() a service
  }
}
...
New tower types, enemy types, or levels are added as entries in definition files — no code changes required elsewhere:
...
// read:
t.worldPosition  // current world position
t.worldRotation  // current world rotation
```
...
Scripts/
  ...
  Defs/                   ← Static data tables (towers, enemies, levels)

Templates/
  Towers/
  Enemies/
  UI/                     ← XAML panels + ViewModels (generated by MHS assistant)
...
| Calling `dispose()` on a `@service()` | Services are singletons — reset state in an event handler instead |
